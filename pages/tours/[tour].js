...
const Details = ({ tours, tour }) => {
  //   const [selectedService, setSelectedService] = useState(null);
  const [selectedPrice, setSelectedPrice] = useState(tour?.low);
                  ...
                  <div className="flex items-center mt-2 w-full">
                    ...
                    >
                      <span className="text-[12px] max-w-[60%]">
                        {"BUGDET"}
                        {console.log(tour?.low)}
                      </span>
                      <span> USD {tour?.low}</span>
                    </label>
                  </div>
                  <div className="flex items-center mt-2  w-full">
                    <input
                    ...
                    >
                      <span className="text-[12px] max-w-[60%]">
                        {"MID-LUXURY"} {console.log(tour?.mid)}
                      </span>
                      <span>USD {tour?.mid}</span>
                    </label>
                  </div>
                  <div className="flex items-center  mt-2 w-full">
                    <input
...
export const getServerSideProps = async (context) => {
  const tourQuery = `*[_type == "tours" && slug.current == "${context.params.tour}"][0]`;
  const sQuery = `*[_type == "tours"]`;

  const tour = await client.fetch(tourQuery);
  const tours = await client.fetch(sQuery);
